...
export function StationDisplay({ stage }: { stage: string }) {
  ...
  if (authErr || !key) {
    return (
      <form
        className="mx-auto max-w-sm space-y-3 rounded-lg border border-stone-200 bg-white p-4"
        onSubmit={(e) => {
          e.preventDefault();
      ...
      >
        ...
        {authErr && <p className="text-sm text-red-600">Key rejected. Try again.</p>}
        <input
          name="k"
          autoFocus
        ...
        />
        <button className="w-full rounded bg-stone-900 py-2 text-white">Unlock station</button>
      </form>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      <div className="flex flex-wrap justify-center gap-2 text-sm">
        {STAGES.map((s) => (
          <Link
            key={s.id}
            href={`/station/${s.id}`}
            className={`rounded px-3 py-1 ${s.id === meta.id ? "bg-stone-900 text-white" : "bg-white border border-stone-200"}`}
          >
            {s.icon} {s.label}
          </Link>
        ))}
      ...
      <div className="rounded-xl bg-white p-6 shadow">
        {data ? <Qr value={data.payload} size={320} /> : <div className="h-80 w-80 bg-stone-100" />}
      </div>
      <div className="w-80">
        <div className="mb-1 flex justify-between font-mono text-sm text-stone-600">
          <span>{data?.code ?? "--------"}</span>
